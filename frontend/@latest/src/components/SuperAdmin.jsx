...
import { 
  ...
  Download, Calendar, TrendingUp, Activity, Shield, Database,
  UserCheck, UserX, Car, Route, MessageSquare, Star, Phone,
  Mail, MoreVertical, Plus, Trash2, RefreshCw, Power, DollarSign
} from 'lucide-react';
...
export default function SuperAdminDashboard() {
  ...
  // New mock data for expanded functionality
  const stats = {
    totalDrivers: 142,
    activeDrivers: 98,
    totalPassengers: 3247,
  ...
  const passengers = [
    { id: 1, name: 'Anil Sharma', phone: '+91 98765 43210', email: 'anil@email.com', trips: 45, lastActive: '2 hours ago', status: 'active' },
    { id: 2, name: 'Meera Patel', phone: '+91 87654 32109', email: 'meera@email.com', trips: 32, lastActive: '1 day ago', status: 'inactive' },
    { id: 3, name: 'Rohit Kumar', phone: '+91 76543 21098', email: 'rohit@email.com', trips: 67, lastActive: 'Just now', status: 'active' },
    { id: 4, name: 'Kavya Singh', phone: '+91 65432 10987', email: 'kavya@email.com', trips: 23, lastActive: '5 hours ago', status: 'active' },
    { id: 5, name: 'Arjun Gupta', phone: '+91 54321 09876', email: 'arjun@email.com', trips: 89, lastActive: '3 days ago', status: 'inactive' }
  ];
  ...
  // New mock data for finance section
  const financialData = {
    weeklyRevenue: [1500, 2000, 1800, 2500, 2200, 2800, 3100],
    payouts: [
      { id: 1, driver: 'Rajesh Kumar', amount: 12400, date: '2024-09-10', status: 'completed' },
      { id: 2, driver: 'Priya Sharma', amount: 11200, date: '2024-09-10', status: 'pending' },
      { id: 3, driver: 'Mohammed Ali', amount: 15600, date: '2024-09-09', status: 'completed' },
      { id: 4, driver: 'Sunita Devi', amount: 10800, date: '2024-09-08', status: 'completed' },
    ]
  };

  const alertData = [
    { id: 1, type: 'emergency', title: 'Route 3B - Emergency Stop', message: 'Driver Priya has triggered an emergency stop.', timestamp: 'Just now' },
    { id: 2, type: 'vehicle', title: 'Vehicle Issue - DL-9A-B-9012', message: 'Driver Mohammed reported engine trouble.', timestamp: '12 min ago' },
    { id: 3, type: 'passenger', title: 'New Complaint', message: 'Passenger reported rude behavior from a driver.', timestamp: '30 min ago' },
  ];
  ...
  const filteredDrivers = filterStatus === 'all' ? drivers : drivers.filter(d => d.status === filterStatus);
  const filteredPassengers = passengers; // A placeholder for future filtering
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="space-y-4">
                {alertData.map(alert => (
                  <div key={alert.id} className={`p-4 rounded-lg flex items-start gap-4 ${getAlertTypeColor(alert.type)}`}>
                    {alert.type === 'emergency' && <AlertTriangle className="h-6 w-6 text-red-600 flex-shrink-0" />}
                    {alert.type === 'vehicle' && <Car className="h-6 w-6 text-orange-600 flex-shrink-0" />}
                    {alert.type === 'passenger' && <MessageSquare className="h-6 w-6 text-blue-600 flex-shrink-0" />}
                    <div>
                      ...
                      <tr key={index} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-gray-900">{passenger.name}</p>
                              <p className="text-sm text-gray-500">Passenger ID: P{String(passenger.id).padStart(4, '0')}</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-gray-900">{financialData.payouts.filter(p => p.status === 'pending').length}</p>
                  </div>
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600">Total Payouts Value</p>
                    <p className="text-2xl font-bold text-gray-900">₹{(financialData.payouts.reduce((sum, p) => sum + p.amount, 0)).toLocaleString()}</p>
                  </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
              <div className="p-6 border-b border-gray-200">
                <h3 className="text-lg font-semibold text-gray-900">Driver Payouts</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50 border-b border-gray-200">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {financialData.payouts.map(payout => (
                      <tr key={payout.id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 text-sm font-medium text-gray-900">{payout.driver}</td>
                        <td className="px-6 py-4 text-sm text-green-600 font-semibold">₹{payout.amount.toLocaleString()}</td>
        ...
        <aside className="w-64 bg-white shadow-sm border-r border-gray-200 min-h-screen">
          <nav className="p-4 space-y-2">
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${
                activeTab === 'finance' ? 'bg-red-50 text-red-600 border border-red-200' : 'text-gray-700 hover:bg-gray-50'
              }`}
            >
